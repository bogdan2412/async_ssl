...
(** Creates either an SSL client or server.
    ...
    [version] is optional, and allows you to customize the specific version of SSL that
    this connection should be using. It may be downgraded - you should check the actually
    negotiated version post-connection.
    ...
    [crt_file] and [key_file] are the on-disk locations of the server's public and private
    keys.

    Any data written into [app_to_ssl] will be encrypted with SSL, and the encrypted data
    will eventually be written into [ssl_to_net].
    ...
    To close the connection and free associated memory, call [Connection.close]. This will
    close all the involved pipes.

    The [session] argument enables the session resumption mechanism: when called with the
    same session value twice, the client will try to resume the session during the second
    call, resulting in a quicker handshake. It is your responsibility to keep the mapping
    between the destinations and the sessions to be used with them. Calling the client
    with the same [session] but with a differet [ca_file] or [ca_path] will result in an
    error. Use [Connection.session_reused] to find out if session resumption actually
    worked.
...
val client
  ...
  -> ?name:string
  -> ?hostname:string
       (** Use [allowed_ciphers] to control which ciphers should be used. See CIPHERS(1),
           and `openssl ciphers -v`.

           If unspecified, [allowed_ciphers] defaults to [`Secure], which is a list
           derived from guidance posted on https://cipherli.st/ and is meant to be
           adjusted over time to reflect current security practices. The current list is
           available via [secure_ciphers].

           If you need to keep a old cipher enabled across an update of the [`Secure]
           cipher list or a default value change[1], you can do something like the
           following:

           [`Only (Ssl.secure_ciphers @ ["OLD_CIPHER"])]

           [1] Historically, [allowed_ciphers] was not a mandatory argument and [`Secure]
           was not the default for backwards-compatibility reasons. However, in early 2018
           [`Secure] became the default to at least force users to think about this if
           they want something less secure. *)
  -> ?allowed_ciphers:[ `Secure | `Openssl_default | `Only of string list ]
  -> ?ca_file:string
  -> ?ca_path:string
  -> ?crt_file:string
  -> ?key_file:string
       (** Use [verify_modes] to control what verification SSL does as part of the
           handshake. To see the full description of the available options see the notes
           on the manpage for SSL_CTX_set_verify(3) on your system.

           Historically, this was an optional argument with no default. However, the fact
           that SSL does *no* verification in that case makes it extremely easy to gain a
           false sense of security regarding the level of verification that is being done,
           so the default is now [Verify_peer].

           To restore the old, insecure, behavior (or to do the verification yourself),
           set this to [Verify_none]. *)
...
val server
  ...
  -> ?ca_path:string
       (** Use [verify_modes] to control what verification SSL does as part of the
           handshake. The default for servers is [Verify_none], meaning no client
           certificate request is sent to the client. *)
  -> ?verify_modes:Verify_mode.t list
       (** Break glass if you are stuck using a certificate/encryption that openssl
           considers insecure. Only set this in exceptional circumstances. *)
  -> ?override_security_level:Override_security_level.t
